 ...
 * map join key, stats).
 *
 * All 249 officially assigned ISO-3166-1 alpha-2 codes. Flag emoji are
 * derived from the code via regional indicator symbols, never hand-typed.
 */
...
/** Regional indicator symbol offset: "A" (0x41) → 🇦 (0x1F1E6). */
const REGIONAL_INDICATOR_OFFSET = 0x1f1e6 - 0x41;

/** Derive the flag emoji for an ISO-3166-1 alpha-2 code (e.g. "FR" → 🇫🇷). */
export function flagEmoji(code: string): string {
  if (!/^[A-Z]{2}$/.test(code)) {
    throw new Error(`flagEmoji expects an uppercase alpha-2 code, got "${code}"`);
  }
...
type CountryData = [code: string, name: string, continent: Continent];

const COUNTRY_DATA: readonly CountryData[] = [
  ["AD", "Andorra", "Europe"],
...
export const COUNTRIES: Country[] = COUNTRY_DATA.map(
  ([code, name, continent]) => ({
    code,
    name,
    continent,
